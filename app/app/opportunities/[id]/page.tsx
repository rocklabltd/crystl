    ...
    <div>
      ...
      {activeTab === "responses" ? (
        <section className="mt-6 space-y-6">
          ...
          {responses.length ? responses.map((response) => <div key={response.id} className="rounded-3xl border border-black/8 bg-white p-6"><div className="flex flex-wrap items-start justify-between gap-4"><div><h2 className="text-lg font-semibold text-neutral-950">{rfqMap.get(response.supplier_rfq_id)?.supplier_name || "Supplier response"}</h2><p className="mt-1 text-sm text-neutral-600">Received {formatDate(response.received_at)}</p></div><div className="flex items-center gap-3"><Link href={`/app/opportunities/${opportunity.id}/responses/${response.id}`} className="text-sm text-neutral-600 underline underline-offset-4">Edit</Link>{response.selected_for_quote ? <span className="rounded-full bg-neutral-950 px-3 py-1 text-xs uppercase tracking-[0.18em] text-white">Preferred</span> : null}</div></div><div className="mt-4 grid gap-3 text-sm text-neutral-700 md:grid-cols-4"><p>MOQ: {response.moq || "Not set"}</p><p>Unit price: {response.unit_price ? `${response.currency} ${response.unit_price}` : "Not set"}</p><p>Lead time: {response.lead_time_days ? `${response.lead_time_days} days` : "Not set"}</p><p>Currency: {response.currency}</p></div><p className="mt-4 text-sm leading-6 text-neutral-600">{response.response_notes || "No notes added."}</p></div>) : <div className="rounded-3xl border border-black/8 bg-white p-8 text-sm text-neutral-600">No supplier responses logged yet.</div>}
        </section>
      ) : null}
      {activeTab === "quotes" ? (
        <section className="mt-6 space-y-6">
          ...
          {quotes.length ? quotes.map((quote) => <div key={quote.id} className="rounded-3xl border border-black/8 bg-white p-6"><div className="flex flex-wrap items-start justify-between gap-4"><div><h2 className="text-lg font-semibold text-neutral-950">{quote.quote_number}</h2><p className="mt-1 text-sm text-neutral-600">{quote.title}</p></div><div className="flex items-center gap-3"><Link href={`/app/opportunities/${opportunity.id}/quotes/${quote.id}`} className="text-sm text-neutral-600 underline underline-offset-4">Edit</Link><span className="rounded-full border border-black/10 px-3 py-1 text-xs uppercase tracking-[0.18em] text-neutral-700">{quote.status}</span></div></div><div className="mt-4 grid gap-3 text-sm text-neutral-700 md:grid-cols-4"><p>Version: V{quote.version_number}</p><p>Price: {quote.unit_price ? `${quote.currency} ${quote.unit_price}` : "Not set"}</p><p>MOQ: {quote.moq || "Not set"}</p><p>Valid until: {quote.valid_until || "Not set"}</p></div></div>) : <div className="rounded-3xl border border-black/8 bg-white p-8 text-sm text-neutral-600">No customer quotes created yet.</div>}
        </section>
      ) : null}
      {activeTab === "activity" ? (
